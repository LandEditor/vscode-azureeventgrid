...
export class EventSubscriptionProvider extends SubscriptionTreeItem {
	...
	public async loadMoreChildrenImpl(
		...
		const listByLocationTasks: Promise<EventSubscription[]>[] = (
			await listLocations(this.root)
			...
			try {
				...
				if (
					parseError(error).errorType === "NoRegisteredProviderFound"
				) {
					return [];
				} else {
					throw error;
	...
	public async createChildImpl(
		...
		actionContext =
			actionContext ||
			<IActionContext>{ properties: {}, measurements: {} };
