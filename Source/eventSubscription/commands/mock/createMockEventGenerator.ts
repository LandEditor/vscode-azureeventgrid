...
export function getEventTypeFromTopic(topic: string): EventType {
	if (
		/^\/subscriptions\/[^\/]+$/i.test(topic) ||
		/^\/subscriptions\/.*\/resourceGroups\/[^\/]+$/i.test(topic)
	) {
...
export async function promptForEventType(): Promise<EventType> {
	const picks: IAzureQuickPickItem<EventType>[] = Object.keys(EventType).map(
		(key: string) => {
			return {
				label: key,
	...
	return (
		await ext.ui.showQuickPick(picks, {
			placeHolder: localize("selectEventType", "Select an event type"),
		})
	).data;
}
